...
Rails.application.configure do
  ...
  # Only attempt delivery once an SMTP host is configured. Without this guard
  # Rails defaults to :smtp against localhost:25, where nothing is listening,
  # and raise_delivery_errors defaults to true — so "forgot my password"
  # returned a 500 rather than failing quietly.
  if ENV["SMTP_ADDRESS"].present?
    ...
    # Nowhere to send mail yet. Swallow the attempt rather than 500 on a
    # password reset; the request still reports success to the user, which is
    # also what Devise does for unknown addresses.
