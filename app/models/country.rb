# The ISO-3166-1 alpha-2 list, used anywhere the app has to name a country:
# a band's own country (Band#country_code, which Stripe Connect requires in
# this format), the destinations a band ships to, and a buyer's shipping
# address.
#
# This is a static table rather than a gem because the app needs nothing a
# gem would add — no translations, no subdivisions, no currency or calling
# codes — and a hard dependency for a frozen hash is not worth carrying.
# Names are the common English short forms, which is what a buyer picking
# from a list expects to read.
class Country
  NAMES = {
  ...
  class << self
    def name_for(code)
      NAMES[normalize(code)]
    end

    def valid?(code)
      NAMES.key?(normalize(code))
    end

    # Ready for a Rails collection select: [ name, code ] pairs sorted by
    # name, so the visible order matches what the reader is scanning.
    def options
      @options ||= NAMES.map { |code, name| [ name, code ] }.sort_by(&:first).freeze
    end
